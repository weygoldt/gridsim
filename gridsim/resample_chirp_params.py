...
import numpy as np
from extract_chirp_params import chirp_model
...
def load_dataset(path: pathlib.Path):
    ...
    print(f"Found {len(freq_fits)} files.")
...
def remove_outliers(dataset):
    ...
    for i in range(dataset.shape[1]):
        param = dataset[:, i]
        outliers = np.abs(param - np.mean(param)) > 2.5 * np.std(param)
...
def resample_chirp_params(path: pathlib.Path):
    ...
    for i, param in enumerate(params):
        ax[i // 4, i % 4].hist(freq_fits[:, i], bins=100, alpha=0.5, label="old")
        ax[i // 4, i % 4].hist(new_freq_fits[:, i], bins=100, alpha=0.5, label="new")
    ...
    for i in range(len(new_freq_fits)):
        ...
        chirp -= mode
        chirp *= tuk
        ax.plot(t, chirp, alpha=0.05, color="k", lw=0.5)
